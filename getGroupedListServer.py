...
def main():
	...
	runOrderList = [[0],[0],[0],[0],[0],[0],[0],[0]]
	motifOrderList = [[0],[0],[0],[0],[0],[0],[0],[0]]
	
	importOrders(runOrderList, motifOrderList)
	clusteredMotifLists = generateSpecUpdMotifLists(runOrderList, motifOrderList, motifLists)
	
	generateUpdLists(clusteredMotifLists)
	generateLabels(clusteredMotifLists)
	
def generateLabels(motifLists):
	os.chdir(sys.argv[1]+'/labels/run')
	for i in range(0,len(motifLists)):
		newFile = open(chooseName(str(i))+'_numbers.txt','w')
		newFile.write('names'+'\n')
		line =''
		for index in range(2,len(motifLists[i][0])):
			line = line + motifLists[i][0][index] + '\n'
		newFile.write(line[:-1])
	
	os.chdir(sys.argv[1]+'/labels/motif')
	for i in range(0,len(motifLists)):
		newFile=open(chooseName(str(i))+'_numbers.txt','w')
		newFile.write('names'+'\n')
		line =''
		for index in range(1,len(motifLists[i])):
			line = line + motifLists[i][index][0] +'\n'
		newFile.write(line[:-1])
		
def generateUpdLists(motifLists):
	for i in range(0,len(motifLists)):
		os.chdir(sys.argv[1]+'/orderedMatrices')
	...
	for i in range(0,len(motifLists)):
		os.chdir(sys.argv[1]+'/orderedMatricesNumbers')
		newFile = open(chooseName(str(i)) + '_numbers.txt','w')
...
def generateSpecUpdMotifLists(runOrderList, motifOrderList, motifLists):
	updMotifLists =[]
	for k in range(0, 8):
		tempList= []
		motifOrderList[k].insert(0,'0')
		for j in motifOrderList[k]:
			newLine = [motifLists[k][int(j)][0],motifLists[0][int(j)][1]]
			for i in runOrderList[k]:
				newLine.append(str(motifLists[k][int(j)][int(i)+1]))
			tempList.append(newLine)
		updMotifLists.append(tempList)
	return updMotifLists

def importOrders(runOrders, motifOrders):
	os.chdir(sys.argv[1]+'/orders')
	for matrix in os.listdir(os.getcwd()):
		index = matrix[0]
		
		motif = True
		if (matrix.find('motif')==-1):
			motif = False
		...
		if (motif):
			motifOrders[int(index)]=splitLine[:-1]
		else:
			runOrders[int(index)]=splitLine[:-1]
...
def generateSpecificMatrix(site, mark, motifList):
	number = 0
	order = ['name','sequence']
	os.chdir(sys.argv[3]+'/'+site+'/'+mark)
	for run in os.listdir(os.getcwd()):
		if (run!='.DS_Store' and run.find(sys.argv[2])==-1):
			os.chdir(sys.argv[3]+'/'+site+'/'+mark+'/'+run)
...
def generateSeparateMatrices(motifLists):
	...
	for siteFolder in os.listdir(os.getcwd()):
		if (siteFolder=='dhsSites' or siteFolder=='allSites'):
			os.chdir(sys.argv[3]+'/'+siteFolder)			
...
def generateInitialLists (namesOfMotifs, sequences, notes):
	...
	for siteFolder in os.listdir(os.getcwd()):
		if (siteFolder=='dhsSites' or siteFolder=='allSites'):
			...
			for markFolder in os.listdir(os.getcwd()):
				if (markFolder!='.DS_Store'):
					os.chdir(sys.argv[3]+'/'+siteFolder+'/'+markFolder)					
					for runFolder in os.listdir(os.getcwd()):
						if (runFolder!='.DS_Store' and runFolder.find(sys.argv[2])==-1):						
							os.chdir(sys.argv[3]+'/'+siteFolder+'/'+markFolder+'/'+runFolder)				
							number = number +1
							if (os.path.isfile('knownResults.txt')):					
								temp = open('knownResults.txt')
								temp.readline()
